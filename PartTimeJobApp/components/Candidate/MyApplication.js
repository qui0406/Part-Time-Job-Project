...
export default function MyApplications() {
    ...
    const loadApplications = async () => {
        try {
            ...
            const requestUrl = `${authApi(token).defaults.baseURL}${endpoints['my-applications']}?status=accepted`;
            console.log('Đang gọi API danh sách:', requestUrl);

            const res = await authApi(token).get(endpoints['my-applications'], {
    ...
    const handleApplicationPress = (application) => {
        ...
        navigation.navigate('RateJob', {
            job: application.job,
            company: application.companyData,
        });
    };
