...
class UserViewSet(viewsets.ViewSet, generics.CreateAPIView):
    ...
    @action(methods=['put', 'patch'], url_path='update-user', detail=False)
...
class CompanyViewSet(viewsets.ViewSet, generics.RetrieveAPIView):
    ...
    def get_current_company(self, request):
        ...
        if not hasattr(request.user, 'employer_profile'):
            return Response({"detail": "User không có company"}, status=status.HTTP_404_NOT_FOUND)
    ...
    def create_current_company(self, request):
        ...
        if hasattr(request.user, 'employer_profile'):
            return Response({"detail": "User đã tồn tại company"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def follow(self, request, pk=None):
        try:
            ...
            if not company.is_approved:
                return Response({"detail": "Không thể tìm thấy công ty follow"}, status=status.HTTP_400_BAD_REQUEST)
...
class CompanyIsApprovedViewSet(viewsets.ViewSet, generics.ListAPIView):
    ...
    def approve_company(self, request, pk=None):
        ...
        if not is_approved:
            return Response({"detail": "Vui long cung cấp đầy đủ thông tin "}, status=status.HTTP_400_BAD_REQUEST)
        try:
            ...
            if is_approved:
                ...
                if employer.role != User.EMPLOYER:
                    employer.role = User.EMPLOYER
                    employer.save()
...
class ApplicationViewSet(viewsets.ViewSet, generics.ListAPIView):
    ...
    def all_accepted_applications(self, request):
        ...
        if not company:
            return Response({"detail": "Bạn không có quyền truy cập vào công ty"}, status=403)
        jobs = Job.objects.filter(company=company, active=True)
        queryset = Application.objects.filter(active=True, job__in=jobs, status='accepted').distinct().order_by('-created_date')
    ...
    def update_status(self, request, pk=None):
        user = request.user
        try: 
            application = Application.objects.filter(pk=pk, active=True)
    ...
    def create_application(self, request):
        ...
        if not job_id:
            return Response({"detail": "Cần job_id"}, status=status.HTTP_400_BAD_REQUEST)
        try:
            job = Job.objects.get(pk=job_id, active=True)
        except Job.DoesNotExist:
            return Response({"detail": "Không tìm thấy job"}, status=status.HTTP_404_NOT_FOUND)
    ...
    def update_application(self, request, pk=None):
        if request.user.role != User.CANDIDATE:
            return Response({"detail": "Chỉ có người nộp mới được cập nhật"}, status=status.HTTP_403_FORBIDDEN)
        try:
        ...
        if application.status != 'pending':
            return Response(
                {"detail": "Chỉ có trạng thái chờ mới được cập nhật"},
                status=status.HTTP_400_BAD_REQUEST
            )
...
class BaseRatingViewSet(viewsets.ViewSet, generics.RetrieveAPIView):
    permission_classes = [permissions.IsAuthenticated]
    def get_permissions(self):
...
class RatingViewSet(BaseRatingViewSet):
    ...
    @action(methods=['post'], url_path='post-rating', detail=False, permission_classes=[permissions.IsAuthenticated])
    def post_rating(self, request):
        ...
        if not job_id:
            raise serializers.ValidationError("Trường job không được để trống.")
        ...
        if serializer.is_valid():
            serializer.save(user=user, company_id=company_id, job_id=job_id)
            return Response({"detail": "Đánh giá đã được gửi thành công"}, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    ...
    def get_rating_from_employer(self, request):
        ...
        if not application_id:
            return Response({"detail": "Cần application id"}, status=status.HTTP_400_BAD_REQUEST)
        queryset = Rating.objects.filter(user=request.user, application_id=application_id)
        serializer = self.get_serializer(queryset, many=True)
        return Response(serializer.data)
    ...
    def get_rating_average(self, request):
        company_id = request.query_params.get('company_id')
        if not company_id:
            return Response({"detail": "Cần company id"}, status=status.HTTP_400_BAD_REQUEST)

        queryset = Rating.objects.filter(company_id=company_id, active=True)
        if not queryset.exists():
            return Response({"detail": "Không tìm thấy đánh giá cho company"}, status=status.HTTP_404_NOT_FOUND)
...
class EmployerRatingViewSet(BaseRatingViewSet):
    ...
    @action(methods=['post'], url_path='post-rating-employer', detail=False, permission_classes=[permissions.IsAuthenticated])
    def post_rating_employer(self, request): 
        employer = request.user
        application_id = request.data.get('application') 


        application_instance = Application.objects.filter(pk=application_id, active=True).first()

        if not application_instance:
            return Response({"detail": "Không tìm thấy đơn ứng tuyển."}, status=404)

        user_id = application_instance.user_id

        if not user_id or not application_id:
            return Response({"detail": "Thiếu dữ liệu người dùng hoặc đơn ứng tuyển."}, status=400)

        serializer = self.get_serializer(data=request.data, context={'request': request})
        if serializer.is_valid():
            try:
                application_instance = Application.objects.get(pk=application_id, user_id=user_id, active=True)
            except Application.DoesNotExist:
                return Response({"detail": "Không tìm thấy đơn ứng tuyển."}, status=404)
            serializer.save(employer=employer, user_id=user_id, application=application_instance)
            return Response({"detail": "Đánh giá đã được gửi thành công"}, status=201)
        return Response(serializer.errors, status=400)


    @action(methods=['get'], url_path='get-notification-rating-user-application', detail=False, permission_classes=[permissions.IsAuthenticated], serializer_class= RatingEmployerWithCommentSerializer)
    def get_notification_user_from_application(self, request):
        employer = request.user
        application_id = request.query_params.get('application_id')
...
class CommentDetailViewSet(viewsets.ViewSet, generics.RetrieveAPIView):
    ...
    def get_all_comments_by_job(self, request):
        ...
        company_id = request.query_params.get('company_id')
        if not job_id:
            return Response({"detail": "Cần job id"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try: 
            ...
            if not comment_details.exists():
                return Response({"detail": "Không tìm thấy bình luận cho công việc này"}, status=status.HTTP_404_NOT_FOUND)

            queryset = Rating.objects.filter(company_id = company_id, job_id = job_id , active=True).order_by('-created_date')
        except Exception as e:
            return Response({"detail": "Không tìm thấy đánh giá cho công việc"}, status=status.HTTP_404_NOT_FOUND)
        page = self.paginate_queryset(queryset)
        if page:
            serializer = self.get_serializer(page, many=True, context={'request': request})
            return self.get_paginated_response(serializer.data)
    ...
    def reply_comment(self, request):
        ...
        if not rating_employer_id:
            return Response(
                {"detail": "Rating_employer_id bắt buộc"},
                status=status.HTTP_400_BAD_REQUEST
            )

        if not employer_reply:
            return Response(
                {"detail": "Reply content băt buộc"},
                status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def update_reply_comment(self, request, pk=None):
        ...
        if not employer_reply:
            return Response(
                {"detail": "Comment bắt buộc"},
                status=status.HTTP_400_BAD_REQUEST
            )

        parent_comment = Rating.objects.filter(pk=pk, active=True).first()
...
class CommentEmployerDetailViewSet(viewsets.ViewSet, generics.RetrieveAPIView):
    ...
    def get_notification_rating(self, request): 
        ...
        queryset = EmployerRating.objects.filter(user_id = user_id,
    ...
    def reply_comment(self, request, pk=None):
        ...
        if not rating_id:
            return Response(
                {"detail": "Rating cadidate ID là bắt buộc"},
                status=status.HTTP_400_BAD_REQUEST
            )

        if not candidate_reply:
            return Response(
                {"detail": "Đánh giá là bắt buộc"},
                status=status.HTTP_400_BAD_REQUEST
            )

        try:
            rating = EmployerRating.objects.get(pk=rating_id)
            
        except EmployerRating.DoesNotExist:
            return Response(
                {"detail": "Không tìm thấy đánh giá cho ứng viên này"},
                status=status.HTTP_404_NOT_FOUND
            )
        ...
        if serializer.is_valid():
            serializer.save(rating_candidate=rating)
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class VerifyDocumentViewSet(viewsets.ViewSet, generics.ListAPIView):
    ...
    def verify_document(self, request):
        ...
        try:
            ...
            return Response({
                'error': 'Lỗi server khi xác minh tài liệu',
                'details': str(e)
            }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def get_status(self, request, pk=None):
        state = False
        try:
            document = VerificationDocument.objects.filter(user=request.user).first()
            if document: 
                state = True
            return Response({
                'state':state
...
class ConversationViewSet(viewsets.ViewSet, generics.ListAPIView):
    ...
    def get_conversation(self, request):
        ...
        try:
            if user.role.__eq__(User.EMPLOYER):
                if not candidate_id:
                    return Response({"detail": "Candidate ID là bắt buộc"}, status=status.HTTP_400_BAD_REQUEST)
                conversation = Conversation.objects.get(employer=user, candidate_id=candidate_id)
            else:
                if not employer_id:
                    return Response({"detail": "Employer ID là bắt buộc"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def create(self, request):
        ...
        if serializer.is_valid():
            ...
            return Response({"message": "Tạo cuộc hội thoại thành công"}, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
